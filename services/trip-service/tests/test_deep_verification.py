...
@pytest.mark.asyncio
async def test_reference_release_after_soft_delete(test_session):
    """Verify that soft-deleted trips (and legacy CANCELLED) are ignored in overlap checks."""
    from trip_service.dependencies import assert_no_trip_overlap
    ...
    trip = TripTrip(
        id="TRIP-SOFT-001",
        trip_no="T1",
        source_type=SourceType.ADMIN_MANUAL.value,
...
@pytest.mark.asyncio
async def test_outbox_atomicity_and_schema(client: AsyncClient, test_session):
    ...
    result = await test_session.execute(select(TripOutbox).where(TripOutbox.event_name == "trip.created.v1"))
    ...
    assert outbox.partition_key == payload_data["id"]
