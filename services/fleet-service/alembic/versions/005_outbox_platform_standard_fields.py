...
Revision ID: 005_outbox_platform_standard_fields
...
revision = "005_outbox_platform_standard_fields"
...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE fleet_outbox ALTER COLUMN payload_json TYPE TEXT USING payload_json::text"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE fleet_outbox ALTER COLUMN payload_json TYPE JSONB USING payload_json::jsonb"
    )
