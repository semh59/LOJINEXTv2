...
from identity_service.models import IdentityOutboxModel, IdentityUserModel
from identity_service.workers.outbox_relay import _process_batch
...
async def _bootstrap_user() -> IdentityUserModel:
    async with async_session_factory() as session:
        result = await session.execute(
            sa.select(IdentityUserModel).where(
                IdentityUserModel.username == "bootstrap-admin"
            )
        )
...
async def test_outbox_failure_does_not_roll_back_other_rows() -> None:
    ...
    assert published_count == 1
    async with async_session_factory() as session:
        failed_row = await session.get(IdentityOutboxModel, failed_id)
