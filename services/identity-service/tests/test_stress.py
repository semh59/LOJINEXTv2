...
from identity_service.token_service import issue_token_pair, seed_bootstrap_state
    ...
    async def stress_crypto():
        ...
        async def _task():
            async with async_session_factory() as local_session:
                ...
                local_user = await local_session.get(IdentityUserModel, user.user_id)
...
async def test_outbox_concurrency_race(session: AsyncSession):
    ...
    from identity_service.token_service import _write_outbox
    ...
    result = await session.execute(select(IdentityOutboxModel))
