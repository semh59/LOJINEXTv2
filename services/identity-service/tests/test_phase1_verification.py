"""Phase 1 Verification Tests — Identity Service.

Validates:
  CRITICAL-01: partition_key column exists and is used by HOL blocking query
  CRITICAL-02: correlation_id and causation_id columns exist on IdentityOutboxModel
...
class TestIdentityOutboxSchema:
    """Verify IdentityOutboxModel has all required traceability columns."""

    def test_partition_key_column_exists(self):
        """CRITICAL-01: partition_key must exist to prevent HOL blocking crash."""
        mapper = inspect(IdentityOutboxModel)
        columns = {c.key for c in mapper.columns}
        assert "partition_key" in columns, (
            "IdentityOutboxModel is missing 'partition_key' column — "
    ...
    def test_correlation_id_column_exists(self):
        ...
        assert "correlation_id" in columns, (
            "IdentityOutboxModel is missing 'correlation_id' — "
    ...
    def test_causation_id_column_exists(self):
        ...
        assert "causation_id" in columns, (
            "IdentityOutboxModel is missing 'causation_id' — event causation chain will be broken"
        )
    ...
    async def test_partition_key_materialized_in_database(self, session):
        ...
        result = await session.execute(
            text(
                ...
                "WHERE table_name = 'identity_outbox' AND column_name = 'partition_key'"
            )
        )
        row = result.scalar_one_or_none()
        assert row == "partition_key", "partition_key column not found in identity_outbox table"
    ...
    async def test_correlation_id_materialized_in_database(self, session):
        ...
        result = await session.execute(
            text(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_name = 'identity_outbox' AND column_name = 'correlation_id'"
            )
        )
        row = result.scalar_one_or_none()
        assert row == "correlation_id", "correlation_id column not found in identity_outbox table"
    ...
    async def test_causation_id_materialized_in_database(self, session):
        ...
        result = await session.execute(
            text(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_name = 'identity_outbox' AND column_name = 'causation_id'"
            )
        )
        row = result.scalar_one_or_none()
        assert row == "causation_id", "causation_id column not found in identity_outbox table"

    @pytest.mark.asyncio
    async def test_partition_index_exists(self, session):
        """Verify ix_identity_outbox_partition index was created."""
        result = await session.execute(
            text(
                "SELECT indexname FROM pg_indexes "
                "WHERE tablename = 'identity_outbox' AND indexname = 'ix_identity_outbox_partition'"
            )
        )
        row = result.scalar_one_or_none()
        assert row is not None, "ix_identity_outbox_partition index missing"
    ...
    async def test_outbox_insert_with_traceability_columns(self, session):
        ...
        result = await session.execute(
            select(IdentityOutboxModel).where(IdentityOutboxModel.outbox_id == "test-outbox-001")
        )
    ...
    async def test_hol_blocking_query_compiles_with_partition_key(self, session):
        ...
        query = (
            select(IdentityOutboxModel.outbox_id)
            .where(
                IdentityOutboxModel.publish_status == "PENDING",
                ~hol_subq.exists(),
            )
            .limit(10)
        )
        # Execute against real DB — should NOT throw
...
class TestIdentityBrokerHeaders:
...
class TestDeadLetterCounterAccuracy:
    ...
    def test_dead_letter_counter_code_pattern(self):
        ...
        relay_path = (
            Path(__file__).parents[1] / "src" / "identity_service" / "workers" / "outbox_relay.py"
        )
