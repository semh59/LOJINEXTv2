...
def _make_outbox_row(
    ...
    return LocationOutboxModel(
        ...
        event_version=1,
        payload_json=payload or {"location_id": "01J000000000000000000ABCDE", "code": "IST"},
...
@pytest.mark.asyncio
async def test_published_envelope_contains_all_required_fields(
    db_engine, override_outbox_settings
) -> None:
    ...
    envelope = kwargs.get("payload") or broker.publish.call_args.args[2] \
        if len(broker.publish.call_args.args) > 2 else None
    ...
    required_fields = {
        ...
        "aggregate_id", "aggregate_type", "payload", "published_at_utc",
    }
...
async def test_published_envelope_field_types(db_engine, override_outbox_settings) -> None:
    ...
    session_factory = async_sessionmaker(db_engine, expire_on_commit=False)
    async with session_factory() as session:
        session.add(_make_outbox_row("location.pair.created.v1",
                                    {"pair_id": "01J000000000000000000PAIR1"}))
        await session.commit()
...
@pytest.mark.asyncio
async def test_envelope_aggregate_type_defaults_to_location(
    db_engine, override_outbox_settings
) -> None:
...
@pytest.mark.asyncio
async def test_envelope_aggregate_id_resolves_from_location_id(
    db_engine, override_outbox_settings
) -> None:
...
@pytest.mark.asyncio
async def test_envelope_aggregate_id_resolves_from_pair_id(
    db_engine, override_outbox_settings
) -> None:
...
@pytest.mark.asyncio
async def test_all_known_event_names_match_naming_convention(
    db_engine, override_outbox_settings
) -> None:
    ...
    async with session_factory() as session:
        for event_name in KNOWN_EVENT_NAMES:
            session.add(LocationOutboxModel(
                event_name=event_name,
                event_version=1,
                payload_json={"location_id": f"01J{event_name[:22].upper().replace('.', '0')}"},
    ...
    for name in published_names:
        assert _EVENT_NAME_RE.match(name), \
            f"Event name {name!r} does not match location.<entity>.<action>.v<n>"
