...
from auth_service.broker import probe_broker
from auth_service.config import settings
from auth_service.database import get_session
from auth_service.models import AuthWorkerHeartbeatModel
...
async def ready(session: AsyncSession = Depends(get_session)) -> JSONResponse:
    ...
    if database_ok:
        heartbeat = await session.get(AuthWorkerHeartbeatModel, OUTBOX_WORKER_NAME)
