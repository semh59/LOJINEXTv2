...
class Settings(BaseSettings):
    ...
    bootstrap_service_clients: str = Field(default="", validation_alias="AUTH_SERVICE_CLIENTS")
    ...
    def service_client_secret_env_name(service_name: str) -> str:
        ...
        return f"AUTH_SERVICE_CLIENT_SECRET__{normalized}"
...
def validate_prod_settings(current: Settings) -> None:
    ...
    if current.auth_jwt_algorithm.upper() != "RS256":
        errors.append("AUTH_AUTH_JWT_ALGORITHM must be RS256 in prod.")
    if current.resolved_broker_type != "kafka":
        errors.append("AUTH_BROKER_TYPE must resolve to kafka in prod.")
    if not current.database_url or current.database_url == DEFAULT_DATABASE_URL:
        errors.append("AUTH_DATABASE_URL must be set to a non-default value in prod.")
    if current.kafka_bootstrap_servers == "localhost:9092":
        errors.append(
            "AUTH_KAFKA_BOOTSTRAP_SERVERS must be set to a non-default value in prod."
        )
    if not current.auth_issuer:
        errors.append("AUTH_AUTH_ISSUER must be set in prod.")
    if not current.auth_audience:
        errors.append("AUTH_AUTH_AUDIENCE must be set in prod.")
    if current.bootstrap_superadmin_password == "change-me-now":
        errors.append("AUTH_BOOTSTRAP_SUPERADMIN_PASSWORD must be overridden in prod.")
    if not current.key_encryption_key_b64:
        errors.append("AUTH_KEY_ENCRYPTION_KEY_B64 must be set in prod.")
    if not current.key_encryption_key_version:
        errors.append("AUTH_KEY_ENCRYPTION_KEY_VERSION must be set in prod.")
    if current.bootstrap_service_clients_json:
        errors.append("AUTH_BOOTSTRAP_SERVICE_CLIENTS_JSON is not allowed in prod.")
    if current.redis_url == "redis://localhost:6379/0":
        errors.append("AUTH_REDIS_URL must be set to a non-default value in prod.")
    if not current.bootstrap_service_names:
        errors.append("AUTH_SERVICE_CLIENTS must list bootstrap service clients in prod.")
