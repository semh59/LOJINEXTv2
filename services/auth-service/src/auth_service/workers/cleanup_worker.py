...
from auth_service.models import AuthAuditLogModel, AuthRefreshTokenModel

logger = logging.getLogger("auth_service.cleanup_worker")
...
async def _purge_expired_refresh_tokens() -> int:
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            delete(AuthRefreshTokenModel)
            .where(AuthRefreshTokenModel.expires_at_utc < cutoff)
            .returning(AuthRefreshTokenModel.token_id)
        )
...
async def _purge_old_audit_logs() -> int:
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            delete(AuthAuditLogModel)
            .where(AuthAuditLogModel.created_at_utc < cutoff)
            .returning(AuthAuditLogModel.audit_id)
        )
        deleted = len(result.fetchall())
        await session.commit()
    return deleted
