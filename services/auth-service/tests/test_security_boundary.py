"""Security boundary tests for auth-service."""
...
async def test_audit_masking_integrity(session: AsyncSession):
    ...
    result = await session.execute(select(AuthAuditLogModel))
...
# ---------------------------------------------------------------------------
# NEW: BUG-4 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Audience bypass is fixed
# ---------------------------------------------------------------------------
...
async def test_audience_bypass_rejected_without_strict_mode(client) -> None:
    ...
    # Must be rejected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â default strict=False means cross-audience is disabled
...
# ---------------------------------------------------------------------------
# NEW: BUG-5 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Retired signing key rejected
# ---------------------------------------------------------------------------
...
async def test_retired_signing_key_rejects_decode(session: AsyncSession) -> None:
    ...
    async with __import__(
        "auth_service.database", fromlist=["async_session_factory"]
...
# ---------------------------------------------------------------------------
# NEW: BUG-2 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Refresh token family reuse detection
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NEW: H-4 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Logout blocklists access token
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NEW: H-4 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â User deactivation revokes refresh tokens
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NEW: BUG-1 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Rate limiting on login endpoint
# ---------------------------------------------------------------------------
...
async def test_rate_limit_login_by_ip(client) -> None:
    ...
    # Send limit+1 requests ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the last one should be 429
    responses = []
