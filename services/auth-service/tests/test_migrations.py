...
from auth_service.crypto import decrypt_private_key, encrypt_private_key

SERVICE_ROOT = Path(__file__).resolve().parents[1]


def _alembic_config(db_url: str) -> Config:
    os.environ["AUTH_DATABASE_URL"] = db_url
...
async def clean_db_url() -> str:
    global_pg_url = os.environ["AUTH_DATABASE_URL"]
...
async def test_signing_key_backfill_requires_kek_env(
    ...
    monkeypatch.delenv("AUTH_KEY_ENCRYPTION_KEY_B64", raising=False)
    monkeypatch.delenv("AUTH_KEY_ENCRYPTION_KEY_VERSION", raising=False)

    with pytest.raises(Exception, match="AUTH_KEY_ENCRYPTION_KEY_B64 is required"):
...
async def test_signing_key_backfill_encrypts_existing_rows(
    ...
    monkeypatch.setenv("AUTH_KEY_ENCRYPTION_KEY_VERSION", "test-v1")
...
async def test_head_migrations_upgrade_on_postgres(
    ...
    outbox_cols = await _get_table_columns(clean_db_url, "auth_outbox")
    assert "outbox_id" in outbox_cols
    audit_cols = await _get_table_columns(clean_db_url, "auth_audit_log")
    assert "audit_id" in audit_cols
    worker_cols = await _get_table_columns(clean_db_url, "auth_worker_heartbeats")
...
async def test_signing_key_downgrade_requires_kek_env(
    ...
    with pytest.raises(
        Exception,
        match="AUTH_KEY_ENCRYPTION_KEY_B64 is required to downgrade signing keys.",
    ):
