"""PDF statement generator — ŞOFÖR PRİM ÇİZELGESİ template.

Layout: 8 trip cards per page (4 rows × 2 columns).
Card rows: NO, TARIH2, SAAT2, PLAKA, GELDİĞİ YER, GİTTİĞİ YER, NET, ÜCRETİ, ONAYI.
ONAYI is left blank for manual signature. SAATİ row is not included.
"""
...
def _card_values(row: StatementRow | None, seq_no: int) -> list[str]:
    ...
    return [
        ...
        net_str,
        row.fee,
        "",  # ONAYI — blank for manual signature
    ]
...
def generate_statement_pdf(
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Şoför Prim Çizelgesi — {driver_name}",
    )
