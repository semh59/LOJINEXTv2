"""Outbox relay worker for reliable event publishing."""
...
async def _process_batch(session: AsyncSession, broker: EventBroker) -> int:
    ...
    # Mark rows as PUBLISHING in a separate atomic step to release the row lock early
    # or keep them locked until the end of the batch.
    # For simplicity and reliability, we keep them locked as we iterate and commit at the end.
    for row in rows:
