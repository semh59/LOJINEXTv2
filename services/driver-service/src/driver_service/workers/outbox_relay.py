...
async def _process_batch(session: AsyncSession, broker: EventBroker) -> int:
    ...
    # Note: We remove PUBLISHING state logic entirely.
    # We use SELECT ... FOR UPDATE SKIP LOCKED to ensure atomic ownership during the transaction.
    query = (
    ...
    for row in rows:
        try:
            ...
            # This call to broker.publish should be async and non-blocking
